...
def basic_unit(x):
    ...
    x = slim.separable_conv2d(x, None, (3, 3), depth_multiplier=1, stride=1, activation_fn=None, scope='depthwise')
...
def basic_unit_with_downsampling(x, out_channels=None):
    ...
    y = slim.separable_conv2d(y, None, (3, 3), depth_multiplier=1, stride=2, activation_fn=None, scope='depthwise')
    y = slim.conv2d(y, out_channels // 2, (1, 1), stride=1, scope='conv1x1_after')

    with tf.variable_scope('second_branch'):
        x = slim.separable_conv2d(x, None, (3, 3), depth_multiplier=1, stride=2, activation_fn=None, scope='depthwise')
